...
const ShowAllSet = () => {
  ...
  const fiteredFoods = useCallback((data: string[]) => {
    return foods
      .filter((el: Food) => data.includes(el.id))
      .reduce((s: number, el: Food) => s + +el.price, 0);
  }, []);

  return (
    <>
      <div
        className="all_set min-h-screen pt-10"
        style={{
          background:
      ...
      >
        <div className="container mx-auto px-12">
          ...
          <ul
            role="list"
            className="divide-y divide-gray-100 mt-10 w-2/3 mx-auto"
          >
            {sets.map((el: Set) => (
              <li
                key={el.id}
                className="flex justify-between gap-x-6 py-5 p-5 items-center w-full text-sm mb-5 font-medium text-gray-900 bg-white border border-gray-300 rounded-lg sm:flex"
              >
                ...
                <button className="text-center bg-transparent hover:bg-red-400 text-red-400 font-semibold hover:text-white py-2 px-4 border border-red-400 hover:border-transparent rounded">
                  <NavLink to={`/set/${el.id}`}>More</NavLink>
                </button>
              </li>
            ))}
          </ul>
